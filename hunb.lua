...
function Menu:Init()
    ...
    task.delay(0.5, function()
        Menu.Notify("<font color='##A6BAFF'>[ Evolution Hub ]</font> — Loaded Successfully | Config Auto-Save Enabled", 5)
    end)
...
function EvoHub:CreateWindow(options)
    options = options or {}
    local name = options.Name or "Evolution Hub"
...
EvoHub.Notify = function(options)
    options = options or {}
    local title = options.Title or "Notification"
    local content = options.Content or ""
    local duration = options.Duration or 3
